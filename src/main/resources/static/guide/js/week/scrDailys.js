    ...
    $.ajax({
        ...
        "success": function (Json) {
            ...
            $('#DataTable tbody').on('click', 'tr td:nth-child(2)', function (e) {
                ...
                var datetime =  $(this).siblings().context.innerHTML;
                //var other = $("#hidden-"+index).val();
                find(index, 2, datetime);
            });
        }
    });
});
...
function find(id, other, datetime) {
    layer.open({
        type: 2,
        shadeClose: true,
        shade: 0.8,
        ...
        fixed: false, //不固定
        scrollbar: false,
        content: '../ScrDailyController/ScrDailyRecord?id=' + id + '&&other=' + other+ '&&datetime=' + datetime
    });
}
        ...
        $.ajax({
            ...
            "success": function (Json) {
                if (Json.data >= 1) {
                    ...
                    setTimeout(function () {
                        window.location.href = "../ScrDailyController/ScrDailys";
                    }, 500);
                } else {
