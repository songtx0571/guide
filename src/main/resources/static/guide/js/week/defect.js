...
function detailedInfoUpd () {
    var defect = {};
    defect.id =  $("#detailedInfoId").text();//id
    defect.abs = $("#detailedInfoAbs").val();//缺陷描述
    defect.method = $("#detailedInfoMethod").val();//处理措施
    defect.problem = $("#detailedInfoProblem").val();//遗留问题
    defect.remark = $("#detailedInfoRemark").val();//备注
    console.log(defect)
    /*$.ajax({
        "type" : 'put',
...
function claimOk() {
    ...
    $.ajax({
        ...
        data: {empIds:empIds,id:id,plannedWork:plannedWork,delayETime:claimInfoBelayTime,delayReason:claimInfoBelay,type: type},
        "success":function(data){
