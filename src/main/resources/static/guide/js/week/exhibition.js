...
function showExhibitionList() {
    ...
    layui.use('table', function () {
        ...
        table.render({
            ...
            , limits: [50, 100, 150]
            , cols: [[ //表头
                {field: 'patrolTask', title: '巡检任务',align: 'center',event: 'task',sort: true,style: 'cursor: pointer;color:blue;'}
...
function addExhibition() {
    ...
    $.ajax({
        ...
        data: {
            'workId': "",
            'planTime': addPlanTime,
            'patrolTask': addTask,
            'cycle': addCycleHidden,
            'department': addDepartNameHidden
        },
        success: function (data) {
            if (data.code == 0 || data.code == 200) {
                layer.closeAll();
...
function updExhibition() {
    ...
    $.ajax({
        ...
        data: {
            'workId': updId,
            'planTime': updPlanTime,
            'patrolTask': updTask,
            'cycle': updCycleHidden,
            'department': updDepartNameHidden
        },
        success: function (data) {
            if (data.code == 0 || data.code == 200) {
                layer.closeAll();
    ...
    layui.use(['form'], function () {
        ...
        form.on('select(addTemBarType)', function (data) {
            ...
            form.on('select(addTemBarSightType)', function (data) {
                ...
                $("#addTemBarSightTypeIdHidden").val(data.value);
            });
        });
        form.on('select(updTemBarType)', function (data) {
            ...
            form.on('select(updTemBarSightType1)', function (data) {
                ...
                $('#updTemBarSightTypeIdHidden').val(data.value);
            });
        });
    })
}
    ...
    layui.use('form', function () {
        ...
        $.ajax({
            ...
            success: function (Tdata) {
                if (Tdata.code == 0 || Tdata.code == 200) {
                    Tdata = Tdata.data;
                    var option = "<option value='-1' >请选择测点类型</option>";
        ...
        $('#updTemBarSightTypeIdHidden').val(data.measuringTypeId);
        $('#updTemBarUnitIdHidden').val(data.unitId);
...
function openTemBar() {
    layui.use('form', function () {
        ...
        $('#addTemBarSightTypeIdHidden').val("");
        $('#addTemBarUnitIdHidden').val("");
        form.render('select');
        form.render(); //更新全部
    });
    $(".temBarDivTable").css("display", "none");
...
function updTemBar() {
    ...
    var sightTypeId = $('#updTemBarSightTypeIdHidden').val();
    var unitTypeId = $('#updTemBarUnitIdHidden').val();
    $.ajax({
        url: '/guide/template/addWorkPeratorChild',
        type: 'GET',
        dataType: 'json',
        ...
        data: {
            ...
            'workId': workId,
            'temChildId': temChildId,
            'sysName': updSystemName,
            'equName': updEquipName,
            'sightTypeId': sightTypeId,
            'unitTypeId': unitTypeId
        },
        success: function (data) {
