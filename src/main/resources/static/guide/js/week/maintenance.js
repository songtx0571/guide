	...
	$.ajax({
		...
		"success": function (Json) {
			...
			if (data == null || data.length == 0){
				...
				for (var i = 0; i < data.length; i ++) {
					tr += "<tr></tr><td>"+(i+1)+"</td><td onclick='showDetailedInfoDiv("+data[i].id+")'>"+data[i].number+"</td><td colspan='4'>"+data[i].abs+"</td><td>"+data[i].empIdsName+"</td><td>"+data[i].realExecuteTime+"</td><td colspan='2'>"+data[i].confirmer1Time+"</td></tr>";
				}
			}
			tbody0.innerHTML = tr;
		}
...
function showDetailedInfoDiv (id) {
	layer.open({
		type: 1,
		title:["缺陷详情页面",'font-size:20px;font-weight:bold;'],
		area: ['750px', '80%'],
		fixed: false, //不固定
		maxmin: true,
