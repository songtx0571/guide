...
public class ScrDailyController {
    ...
    public Result getTeamOpeartionLog(
        ...
        if(type!=null){
            if (type == 1) {
                startTime = sdf.format(DateFormat.getThisDayTimeBegin(parseDate, 0, 8));
                endTime = sdf.format(DateFormat.getThisDayTimeBegin(parseDate, 0, 16));
            } else if (type == 2) {
                startTime = sdf.format(DateFormat.getThisDayTimeBegin(parseDate, 0, 16));
                endTime = sdf.format(DateFormat.getThisDayTimeBegin(parseDate, 1, 0));
            } else if (type == 3) {
                startTime = sdf.format(DateFormat.getThisDayTimeBegin(parseDate, 0, 0));
                endTime = sdf.format(DateFormat.getThisDayTimeBegin(parseDate, 0, 8));
            }
        }
        ...
        if (defectList != null && defectList.size() > 0) {
            for (Defect defect : defectList) {
                ...
                map.put("abs", "创建缺陷" + defect.getAbs());
